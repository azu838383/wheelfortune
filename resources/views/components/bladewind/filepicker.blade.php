...
@endif
<div
    class="relative border-2 border-dashed border-gray-300 dark:text-dark-300 dark:border-dark-700
    dark:bg-dark-800 hover:dark:border-dark-600 text-center cursor-pointer rounded-md flex items-center justify-center {{ $selected_value_class }} bw-fp-{{ $name }} @if ($add_clearing) mb-3 @endif">
    ...
    <x-bladewind::icon name="x-circle"
        ...
        type="solid" />
    <div id="preview" class="text-gray-400/80 selection flex items-center justify-center h-full w-3/5">
        {{ $placeholder }}
        @if ($required)
    ...
    <div class="w-0 h-0 overflow-hidden">
        <input type="file" name="{{ $name }}"
            class="bw-{{ $name }} @if ($required) required @endif" id="bw_{{ $name }}"
            accept="{{ $accepted_file_types }}" />
        <textarea class="b64-{{ $name }}@if ($required) required @endif"
            name="b64_{{ $name }}"></textarea>
        @if (!empty($selected_value))
            <input type="hidden" class="selected_{{ $name }}" name="selected_{{ $name }}"
...
<script>
    ...
    @if (!empty($url))
        @if (in_array(pathinfo($url, PATHINFO_EXTENSION), $image_file_types))
            file =
                '<img src="{{ $url }}" alt="{{ $url }}" class="rounded-md object-contain {{ $selected_value_class }}" />';
        @else
