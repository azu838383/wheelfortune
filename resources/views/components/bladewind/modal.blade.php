...
<div data-name="{{$name}}" data-backdrop-can-close="{{$backdrop_can_close}}"
    ...
    <div class="sm:{{$sizes[$size]}} w-full p-4 m-auto bw-{{$name}} animate__faster">
        <div class="bg-slate-900 border border-slate-800 rounded-lg drop-shadow-2xl">
            @if( $show_action_buttons && $show_close_icon)
                <a href="javascript:void(0)" onclick="{!! $cancelAction !!}">
                    <x-bladewind::icon name="x-mark"
                                       class="text-gray-400 hover:rounded-full hover:bg-slate-800 p-1 absolute right-3 top-3 modal-close-icon"/>
                </a>
            @endif
            <div class="{{(!empty($type))?'flex':'flex-initial'}}">
                ...
                <div class="modal-body grow p-7 @if(!empty($type)) !pl-3 @endif {{ $body_css  }}">
                    <h1 class="text-[22px] font-bold text-slate-300 modal-title text-left">{{ $title }}</h1>
                    <div class="modal-text text-gray-400 pt-2 text-base leading-6 tracking-wide text-left">
                        {{ $slot }}
                    </div>
                </div>
            </div>
            @if( $show_action_buttons )
                <div class="modal-footer @if($stretch_action_buttons) flex flex-col-reverse @endif @if($center_action_buttons || in_array($size, ['tiny', 'small', 'medium'])) text-center @else text-right @endif bg-slate-800/50 border-t border-slate-800 py-3 px-6 rounded-br-lg rounded-bl-lg {{ $footer_css }}">
                    <x-bladewind::button
                            type="secondary"
                            size="{{$button_size}}"
                    ...
                    <x-bladewind::button
                            size="{{$button_size}}"
                            onclick="{!! $okAction !!}"
                            class="okay !bg-gradient-to-r from-yellow-600 to-yellow-300 !text-black font-bold {{ (($stretch_action_buttons) ? 'block w-full mb-3 !ml-0' : 'ml-3') }} {{ $okCss }}">{{$ok_button_label}}</x-bladewind::button>
                </div>
            @endif
        </div>
    </div>
