...
@php
    ...
    $action_icons = !empty($action_icons)
        ? (is_array($action_icons)
            ? $action_icons
            : json_decode(str_replace('&quot;', '"', $action_icons), true))
        : [];
    $column_aliases = !empty($column_aliases)
        ? (is_array($column_aliases)
            ? $column_aliases
            : json_decode(str_replace('&quot;', '"', $column_aliases), true))
        : [];
    $icons_array = [];
    ...
    if (!empty($data)) {
        ...
        if (!empty($exclude_columns)) {
            $table_headings = array_filter($table_headings, function ($column) use ($exclude_columns) {
...
<div class="z-20"> {{-- max-w-screen overflow-x-hidden md:w-full --}}
    <div class="w-full">
        @if ($searchable)
            <div class="bw-table-filter-bar flex justify-end">
                <div class=" {{ $class_search }}">
                    <x-bladewind.input name="bw-search-{{ $name }}" placeholder="{{ $search_placeholder }}"
                        onkeyup="filterTable(this.value, 'table.{{ $name }}')" prefix_is_icon="true"
                        add_clearing="false" class="!mb-0 rounded-full overflow-hidden search-input" prefix="magnifying-glass" />
                </div>
            </div>
        @endif
        <table
            class="bw-table w-full {{ $name }} @if ($has_shadow) shadow-2xl shadow-gray-200 dark:shadow-xl dark:shadow-slate-900 @endif
                                ...
                                @if (!empty($icons_array))
                                    <td class="text-right space-x-2 actions">
                                        @foreach ($icons_array as $icon)
                                            @if (isset($icon['icon']))
                                                <x-bladewind::button.circle size="tiny" icon="{{ $icon['icon'] }}"
                                                    color="{{ $icon['color'] ?? '' }}"
                                                    tooltip="{{ $icon['tip'] ?? '' }}" onclick="{!! build_click($icon['click'], $row) ?? 'void(0)' !!}"
